...
fn resolve_summary_window(
    ...
    let window = match selection {
        ...
        SummarySelection::Month => {
            let month_start = today - Duration::days(30);
            SummaryWindow {
                from: month_start,
                to: today,
...
mod tests {
    ...
    fn resolves_week_window_as_last_7_days() {
        ...
        assert_eq!(window.from, NaiveDate::from_ymd_opt(2026, 3, 5).unwrap());
        assert_eq!(window.to, today);
        assert_eq!(window.date_label, "2026-03-05 to 2026-03-12 (last 7 days)");
